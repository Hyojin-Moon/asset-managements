...
import type { BudgetItem, ExpenseCategory, PersonType } from '@/types'

const TABS: (PersonType | '전체')[] = ['전체', '공통', '효진', '호영', '정우']

export function ExpensesClient({ items: initialItems, categories }: { items: BudgetItem[]; categories: ExpenseCategory[] }) {
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-border text-muted-foreground">
                      ...
                      <th className="text-left py-3 px-3 font-medium">카테고리</th>
                  ...
                  <tbody>
                    {filtered.map((item) => (
                      <SortableTableRow key={item.id} id={item.id}>
                        ...
                        <td className="py-3 px-3 text-right font-semibold text-primary-dark">
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-border">
                      <td colSpan={2} />
                      <td colSpan={3} className="py-3 px-3 font-semibold">합계</td>
                      <td className="py-3 px-3 text-right font-bold text-primary-dark">{formatKRW(total)}</td>
                      <td colSpan={3} />
                    </tr>
                  </tfoot>
                </table>
                  ...
                  <SortableMobileCard key={item.id} id={item.id} className="flex items-center gap-2 p-3 rounded-xl bg-muted/50">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {item.category_name && (
                          <Badge className="text-[10px]">{item.category_name}</Badge>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-semibold text-sm text-primary-dark">{formatKRW(item.amount)}</span>
                      <button
                        onClick={() => { setEditItem(item); setFormOpen(true) }}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {dupConfirm.duplicateNames.map((name) => (
                <span
                  key={name}
                  className="px-3 py-1.5 rounded-lg bg-primary-bg text-primary-dark text-sm font-medium"
                >
                  {name}
                </span>
              ))}
